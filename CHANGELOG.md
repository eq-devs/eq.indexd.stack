...
### 🐛 Fixes
- **`LazyStackController.currentIndex` going out of range now fails loudly in debug mode instead of silently blanking a tab.** The controller has no reference to `LazyLoadIndexedStack.children.length` — `switchTo`/`preloadPage`/`preloadAdjacentPages` all take a caller-supplied `totalPages` with no cross-check against the widget's actual child count. A stale/miscalculated `totalPages`, or a tab count that shrinks at runtime without updating it, let `currentIndex` end up pointing past the end of `children`; `RenderLazyStack` would then silently paint nothing and stop responding to hit-tests for that page, with no error anywhere. Added a debug-mode `assert` (zero cost in release) at both initial mount and every controller-driven index change.
- **Fixed a `mounted` guard asymmetry in `_onControllerChanged`**: the index-changed branch checked `mounted` before touching state; the branch for other controller notifications (e.g. `preloadPage`) did not, before writing to `_buildVersion` (a `ValueNotifier` disposed alongside the `State`). No reproducible crash was found under normal widget lifecycle (`dispose()` removes the listener synchronously before anything else), but the guard now covers both branches for defensive symmetry.

### 📝 Docs
- Fixed three places in `README.md` that incorrectly documented `scaleIn`'s default `paintOrder` as `IndexdPaintOrder.outgoingOnTop` — the actual default (and the fourth README mention) is `IndexdPaintOrder.stack`.
